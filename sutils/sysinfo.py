...
def get_macid():
    ...
    for iface, info in ifaces.items():
        ...
        macs.sort()
        mac_str = ':'.join(macs)
        hash_str = hash.md5(mac_str.encode('UTF-8')).hexdigest()
    return hash_str

def get_ips():
    ifaces = psutil.net_ifaddrs()
    ips = list()
    for iface, info in ifaces.items():
        if iface == 'lo':
            continue
...
def get_cpu():
    ...
    cpu_precent = [str(cpu)+'%' for cpu in cpu_percent]
...
def get_network():
    ...
    for k, v in ifaces.items():
        ...
        ifnet['recv'] = '%.2fMB'%(data.bytes_sent/1024/1024)
