# coding:UTF-8
# @Author: Hilbert
# @Datetime: 2019-3-12 15:03:10
# @Version:1.0
# @File control.py
# @Description: The module is nodes control ,get resource of every node, implement loading leveling
...
class NodeInfo():
    def __init__(self):
        self.pool = redis.ConnectionPool(host=RTASK_REDIS_HOST, port=RTASK_REDIS_POST, db=RTASK_REDIS_DB,
                                         password=RTASK_REDIS_PWD, encoding='utf-8', decode_responses=True)
    ...
    def rpcconn_check(self, ip):
        try:
            rpc = zerorpc.Client(timeout=3, hearbeat=3)
    ...
    def task_list(self):
        tasks = self.client.hgetall(TASK_NAME+'task status')
    ...
    def clear_data(self):
        ...
        self.client.delte(TASK_NAME+':task_status')
...
class WorkerControl():
    ...
    def __init__(self, ip):
        ...
        self.rpc.connecct("tcp://{ip}:{port}".format(ip=ip, port=RPC_PORT))
    ...
    def kill_worker(self):
        result = self.rpc.kill_worder(RPC_PWD)
        return result
        ...
        if isinstance(clients, list):
            ...
            for client in clients:
                ...
                for k,v in client.items():
                    temps.append(k+'='+v)
                client_lists.append(' '.joing(temps))
            return client_lists
        else:
            client_dicts = dict()
            for ip, clients in clients.items():
                ...
                for client in clients:
                    ...
                    for k, v in client.items():
                        temps.append(k+"="+v)
                    client_dicts.append(' '.joing(temps))
                return client_dicts
...
# node server system information
class ServerInfo():
